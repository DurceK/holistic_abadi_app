import 'dart:convert'; //Importando os pacotes necessários para a criação desta instância.
import 'dart:async';
...
class _PedidoPageState extends State<PedidoPageTest> {
  ...
  Future<List<Base>>_getBases()async{
    ...
    for(var b in jsonData){
      Base base = Base(b["bases"]["0"]["base"], b["bases"]["0"]["descricao"], b["bases"]["0"]["status"], b["bases"]["0"]["totPedDia"], b["bases"]["0"]["totPedDiaFaturados"], b["bases"]["0"]["totPedDiaCarregados"], b["bases"]["0"]["totPedDiaMenosUm"], b["bases"]["0"]["totPedDiaMenosDois"], b["bases"]["0"]["totPedDiaMenosTres"], b["bases"]["0"]["totPedDiaMenosQuatro"]);
      bases.add(base);
    }
    print(bases.length);
...
class Base {
  ...
  Base( //Método Set da classe Bases().
      this.base,
      this.descricao,
      this.status,
      this.totPedDia,
